...
class UICamera(*uic.loadUiType(ui_path)):
    def __init__(self,
        ...
        self.push_update_parking.clicked.connect(self.update_parking)
        ...
        # self.spinBox_zero_x.setValue(self.settings.value('sample_stage_park_x_pix', defaultValue=250, type=int))
        # self.spinBox_zero_y.setValue(self.settings.value('sample_stage_zero_y_pix', defaultValue=250, type=int))

        sample_stage_park_x_rbk = self.settings.value('sample_stage_park_x_rbk', defaultValue=0, type=float)
        sample_stage_park_y_rbk = self.settings.value('sample_stage_park_y_rbk', defaultValue=0, type=float)

        self.spinBox_park_x_rbk.setValue(sample_stage_park_x_rbk)
        self.spinBox_park_y_rbk.setValue(sample_stage_park_y_rbk)

        self.sample_positioner = SamplePositioner(self.RE,
                                                  self.sample_stage,
                                                  sample_stage_park_x_rbk,
                                                  sample_stage_park_y_rbk)
    ...
    def stage_left(self):
        h_step = self.spinbox_ver_step.value()
        self.RE(bps.mvr(self.sample_stage.x, h_step))
        self.show_image()
    ...
    def update_parking(self):
        ...
        ret = question_message_box(self, 'Stage Parking Update',
                                   ('Are you sure you want to update stage parking position?\n' +
                                    'You may need to recalibrate the sample positioning'))
        if ret:
            sample_x_park_pos = self.sample_stage.x.read()[self.sample_stage.x.name]['value']
            sample_y_park_pos = self.sample_stage.y.read()[self.sample_stage.y.name]['value']

            self.spinBox_park_x_rbk.setValue(sample_x_park_pos)
            self.spinBox_park_y_rbk.setValue(sample_y_park_pos)

            self.settings.setValue('sample_stage_park_x_rbk', self.spinBox_park_x_rbk.value())
            self.settings.setValue('sample_stage_park_y_rbk', self.spinBox_park_y_rbk.value())
