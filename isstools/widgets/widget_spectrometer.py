...
class UISpectrometer(*uic.loadUiType(ui_path)):
    def __init__(self,
        ...
        #
        #
        self.push_tune_focusing.clicked.connect(self.tune_focusing)
    ...
    #
    #     self.push_spectrometer.clicked.connect(self.run_spectrometer)
    #     self.push_spectrometer_save.clicked.connect(self.save_spectrometer)
    #     self.push_tune_beamline.clicked.connect(self.tune_beamline)
    #
    #     self.last_text = '0'
    #     self.tune_dialog = None
    #     self.last_spectrometer_uid = ''
    ...
    #
    #     self.cid_spectrometer = self.canvas_spectrometer.mpl_connect('button_press_event', self.getX_spectrometer)
    #
    ...
    #
    def addCanvas(self):
        self.figure_spectrometer = Figure()
        self.figure_spectrometer.set_facecolor(color='#FcF9F6')
        self.canvas_spectrometer = FigureCanvas(self.figure_spectrometer)
        self.canvas_spectrometer.motor = ''
        self.figure_spectrometer.ax = self.figure_spectrometer.add_subplot(111)
        self.toolbar_spectrometer = NavigationToolbar(self.canvas_spectrometer, self, coordinates=True)
        self.plot_spectrometer.addWidget(self.toolbar_spectrometer)
        self.plot_spectrometer.addWidget(self.canvas_spectrometer)
        self.canvas_spectrometer.draw_idle()
        self.cursor_spectrometer = Cursor(self.figure_spectrometer.ax, useblit=True, color='green', linewidth=0.75)
        self.figure_spectrometer.ax.grid(alpha=0.4)
    #
    # def run_spectrometer(self, **kwargs):
    ...
    #     detectors = []
    #
    #     self.canvas_spectrometer.mpl_disconnect(self.cid_spectrometer)
    #
    ...
    #             self.canvas_spectrometer.motor = curr_mot
    #             break
    #
    #     if curr_det == '':
    ...
    #
    #     if not repeat:
    #         update_figure([self.figure_spectrometer.ax], self.toolbar_spectrometer,self.canvas_spectrometer)
    #
    #
    ...
    #     print(curr_mot)
    #
    #     self.push_spectrometer.setEnabled(False)
    #     try:
    ...
    #                                            num_steps, False,
    #                                            retries=1,
    #                                            ax=self.figure_spectrometer.ax))
    ...
    #         uid_list = []
    #
    #     self.figure_spectrometer.tight_layout()
    #     self.canvas_spectrometer.draw_idle()
    #     self.cid_spectrometer = self.canvas_spectrometer.mpl_connect('button_press_event', self.getX_spectrometer)
    #
    #     self.push_spectrometer.setEnabled(True)
    #     self.last_spectrometer_uid = self.db[-1]['start']['uid']
    #     self.push_spectrometer_save.setEnabled(True)
    #
    # def save_spectrometer(self):
    #     run = self.db[self.last_spectrometer_uid]
    ...
    #     ydata = []
    #     xdata = []
    #     for line in self.figure_spectrometer.ax.lines:
    ...
    #                comments=comments)
    #
    # def getX_spectrometer(self, event):
    #     if event.button == 3:
    #         if self.canvas_spectrometer.motor != '':
    #             dlg = MoveMotorDialog.MoveMotorDialog(new_position=event.xdata, motor=self.canvas_spectrometer.motor,
    #                                                   parent=self.canvas_spectrometer)
    #             if dlg.exec_():
    #                 pass
    #
    def tune_focusing(self):
        # self.canvas_spectrometer.mpl_disconnect(self.cid_spectrometer)
        # self.canvas_spectrometer.motor = ''
        print(f'[PCL tuning] Starting...', file=self.parent_gui.emitstream_out, flush=True )
        ...
        for element in self.tune_pcl:
            print(f'[Focusing tuning] {element["comment"]}')
            ...
            self.RE(self.aux_plan_funcs['tuning_scan'](motor, detector,
                    ...
                    LivePlot(detector.hints['fields'][0], x=motor.name, ax=self.figure_spectrometer.ax))
        ...
        print('[Focusing tuning] Focusing tuning complete',file=self.parent_gui.emitstream_out, flush=True)
    #
    #
