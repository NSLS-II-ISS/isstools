...
class TrajectoryStack:
    def __init__(self, hhm, trajectory_manager):
    ...
    def set_traj(self, input):
        if type(input) == dict:
            ...
            if self.current_traj_slot != slot_number:
                self.trajectory_manager.init(slot_number)
    ...
    def create_new_trajectory(self, traj_signature, traj_index): # creates, saves, loads, and initializes trajectory with this signature
        ...
        trajectory_creator = TrajectoryCreator(self.hhm.servocycle, self.hhm.pulses_per_deg)
        if traj_signature['type'] == 'Double Sine':
            ...
            trajectory_creator.define(edge_energy=traj_signature['parameters']['E0'],
