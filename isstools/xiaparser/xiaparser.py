...
class xiaparser:
            ...
            for ds_index, ds in enumerate(self.data):
                for board_number, curr_ds in enumerate(ds):
                    ...
                    for i in range(number_pixels):
                        self.next_pos = self.read_pixel_block(
                            curr_ds,
                            board_number,
                            i,
                            self.next_pos,
                    ...
                    for index, channel in enumerate(
                        [channel_num + board_number * 4 for channel_num in range(4)]
                    ):
                        self.exporting_arrays[channel] = (
                            self.exporting_arrays[channel] + self.chans[index]
                        )
    ...
    def export_files(self, dest_filepath, dest_filename="", all_in_one=True):
        ...
        while os.path.isfile(dest_filepath + tmpfilename):
            tmpfilename = (
                self.filename[0 : len(self.filename) - 3] + "-" + str(try_number)
            )
            try_number += 1
        ...
        with warnings.catch_warnings():
            ...
            try:
                if all_in_one:
                    ...
                    f = h5py.File(
                        dest_filepath + tmpfilename + "-allchans.hdf5", mode="w"
                    )
                    f.create_dataset(
                        "AllChans", data=self.exporting_arrays, compression="gzip"
                    )
                    ...
                    for index, array in enumerate(self.exporting_arrays):
                        ...
                        f = h5py.File(
                            "{}{}-chan{}.hdf5".format(
                                dest_filepath, tmpfilename, index
                            ),
                            mode="a",
                        )
                        f.create_dataset(
                            "Chan{}".format(index), data=array, compression="gzip"
                        )
    ...
    def export_files_txt(self, dest_filepath, dest_filename="", all_in_one=True):
        ...
        if all_in_one:
            ...
            for index, array in enumerate(exporting_arrays):
                print("Creating file for channel {}...".format(index))
                np.savetxt(
                    "{}{}-chan{}.txt".format(dest_filepath, tmpfilename, index),
                    array,
                    fmt="%i",
                    delimiter=" ",
                )
    ...
    def read_pixel_block(
        self, dataset, board_number, pixel, start_pos, plot_data, pixel_to_parse, silent
    ):
        def read_statistics(ds, start):
            ...
            realtime = (
                self.two_word_to_int(ds[pos], ds[pos + 1]) * 320 / 1000000
            )  # 320ns
            pos += 2
            livetime = (
                self.two_word_to_int(ds[pos], ds[pos + 1]) * 320 / 1000000
        ...
        mapping_mode = dataset[
            pos + 3
        ]  # 1 = Full Spectrum, 2 = Multiple ROI, 3 = List Mode
        pixel_number = self.two_word_to_int(dataset[pos + 4], dataset[pos + 5])
        total_pixel_block_size = self.two_word_to_int(
            dataset[pos + 6], dataset[pos + 7]
        )
        ...
        (
            channel_0_stats_realtime,
            channel_0_stats_livetime,
            channel_0_stats_triggers,
            channel_0_stats_output_evs,
        ) = read_statistics(dataset, pos + 32)
        (
            channel_1_stats_realtime,
            channel_1_stats_livetime,
            channel_1_stats_triggers,
            channel_1_stats_output_evs,
        ) = read_statistics(dataset, pos + 40)
        (
            channel_2_stats_realtime,
            channel_2_stats_livetime,
            channel_2_stats_triggers,
            channel_2_stats_output_evs,
        ) = read_statistics(dataset, pos + 48)
        (
            channel_3_stats_realtime,
            channel_3_stats_livetime,
            channel_3_stats_triggers,
            channel_3_stats_output_evs,
        ) = read_statistics(dataset, pos + 56)

        reserved = dataset[pos + 64 : 256]  # 255]  # reserved (set to 0)

        channel_0_spectrum = dataset[
            pos + 256 : pos + 256 + K - 1 + 1
        ]  # Python gets range - 1
        channel_1_spectrum = dataset[pos + 256 + K : pos + 256 + K + L - 1 + 1]
        channel_2_spectrum = dataset[pos + 256 + K + L : pos + 256 + K + L + M - 1 + 1]
        channel_3_spectrum = dataset[
            pos + 256 + K + L + M : pos + 256 + K + L + M + N - 1 + 1
        ]
        ...
        if (pixel_number == number or pixel_to_parse == None) and (not silent):
            ...
            print(
                "Channel {} Realtime: {} ms".format(
                    (4 * board_number) + 0, channel_0_stats_realtime
                )
            )
            print(
                "Channel {} Livetime: {} ms".format(
                    (4 * board_number) + 0, channel_0_stats_livetime
                )
            )
            print(
                "Channel {} Triggers: {}".format(
                    (4 * board_number) + 0, channel_0_stats_triggers & 0xFFFF
                )
            )
            print(
                "Channel {} Output Events: {}".format(
                    (4 * board_number) + 0, channel_0_stats_output_evs & 0xFFFF
                )
            )
            print(
                "Channel {} Spectrum: {}\n".format(
                    (4 * board_number) + 0, channel_0_spectrum
                )
            )
            ...
            print(
                "Channel {} Realtime: {} ms".format(
                    (4 * board_number) + 1, channel_1_stats_realtime
                )
            )
            print(
                "Channel {} Livetime: {} ms".format(
                    (4 * board_number) + 1, channel_1_stats_livetime
                )
            )
            print(
                "Channel {} Triggers: {}".format(
                    (4 * board_number) + 1, channel_1_stats_triggers & 0xFFFF
                )
            )
            print(
                "Channel {} Output Events: {}".format(
                    (4 * board_number) + 1, channel_1_stats_output_evs & 0xFFFF
                )
            )
            print(
                "Channel {} Spectrum: {}\n".format(
                    (4 * board_number) + 1, channel_1_spectrum
                )
            )
            ...
            print(
                "Channel {} Realtime: {} ms".format(
                    (4 * board_number) + 2, channel_2_stats_realtime
                )
            )
            print(
                "Channel {} Livetime: {} ms".format(
                    (4 * board_number) + 2, channel_2_stats_livetime
                )
            )
            print(
                "Channel {} Triggers: {}".format(
                    (4 * board_number) + 2, channel_2_stats_triggers & 0xFFFF
                )
            )
            print(
                "Channel {} Output Events: {}".format(
                    (4 * board_number) + 2, channel_2_stats_output_evs & 0xFFFF
                )
            )
            print(
                "Channel {} Spectrum: {}\n".format(
                    (4 * board_number) + 2, channel_2_spectrum
                )
            )
            ...
            print(
                "Channel {} Realtime: {} ms".format(
                    (4 * board_number) + 3, channel_3_stats_realtime
                )
            )
            print(
                "Channel {} Livetime: {} ms".format(
                    (4 * board_number) + 3, channel_3_stats_livetime
                )
            )
            print(
                "Channel {} Triggers: {}".format(
                    (4 * board_number) + 3, channel_3_stats_triggers & 0xFFFF
                )
            )
            print(
                "Channel {} Output Events: {}".format(
                    (4 * board_number) + 3, channel_3_stats_output_evs & 0xFFFF
                )
            )
            print(
                "Channel {} Spectrum: {}".format(
                    (4 * board_number) + 3, channel_3_spectrum
                )
            )
    ...
    def read_header(self, dataset, print_headers, silent):
        ...
        seq_buffer_number = self.two_word_to_int(
            dataset[5], dataset[6]
        )  # Seq Buffer Number (low word first)
        buffer_id = dataset[7]  # BufferID (0: A, 1:B)
        number_of_pixels_in_buffer = dataset[8]  # Number of Pixels in buffer

        starting_pixel_number = self.two_word_to_int(
        ...
        buffer_errors = dataset[
            24
        ]  # Buffer errors: Buffer overrun 0: No error / >0: Number of extra pixels combined with last pixel in buffer
    ...
    def parse_roi(
        self, pixels, channel_number, rois=[[0, 20]], energy_range=20
    ):  # min_energy = 0, max_energy = 20):
        roi_integrations = []
        ...
        for roi in rois:
            ...
            roi_integrations.append(
                list(
                    [
                        v.values[channel_number - 1] @ condition.astype(int)
    ...
    def plot_roi(
        ...
        parsed_roi_array = self.parse_roi(
            pixels, channel_number, min_energy, max_energy
        )
        ...
        if len(energy_array) == len(parsed_roi_array):
            ...
            if len(energy_array):
                print(
                    "The parsed ROI array and the energy array have different lengths.. \nPlotting only parsed_roi_array\nenergy_array length: {}\nparsed_roi_array length: {}".format(
                        len(energy_array), len(parsed_roi_array)
                    )
                )
        ax.grid(True)
    ...
    def gain_matching(self, xia, center_energy, scan_range, channel_number, ax=plt):
        ...
        condition = (graph_x <= (center_energy + scan_range) / 1000) == (
            graph_x > (center_energy - scan_range) / 1000
        )
    ...
    def parse_step_scan(set_name, roi_start, roi_end, path="/GPFS/xf08id/xia_files/"):
        ...
        energies, i0_values = np.loadtxt(
            "{}{}-{}".format(path, set_name, "i0")
        ).transpose()
        ...
        det_channels.append(
            np.array(det_channels[2])
            + np.array(det_channels[3])
            + np.array(det_channels[4])
            + np.array(det_channels[5])
        )
